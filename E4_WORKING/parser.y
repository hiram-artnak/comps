...
variable_declaration: type identifier_list {
    ...
    for(int i = 0; i < size; i++){
        ...
        symbol *sym = symbol_create($1, SYMBOL_TYPE_IDENTIFIER, ast_node_get_lexeme(cur_node));
...
function: function_header function_body {
        ast_node *node = $1; 
        ast_node_add_child(node, unwind_ast_node_list($2)); 
        $$ = node; 
}
    ;
...
function_header: parameter_list TK_OC_GE type '!' identifier {
    ...
    symbol *sym = symbol_create($3, SYMBOL_TYPE_FUNCTION, ast_node_get_lexeme($5)); 
    add_symbol_to_current_scope(scope_stack, sym); 
    ...
    for(int i = 0; i < size; i++){ 
        ...
        fail_if_declared(scope_stack, cur_node); 
        symbol *sym = symbol_create(ast_node_get_data_type(cur_node), SYMBOL_TYPE_IDENTIFIER, ast_node_get_lexeme(cur_node)); 
        add_symbol_to_current_scope(scope_stack, sym); 
    } 
}
;
...
command: command_block ';' { $$ = unwind_ast_node_list($1);}
...
function_call: identifier '(' arguments ')'{
    ...
    ast_node_add_child(node, unwind_ast_node_list($3));
    $$ = node;
};
...
while_command: TK_PR_WHILE '(' expression ')' command_block{
    ...
    ast_node_add_child(node, $3);
    ast_node_add_child(node, unwind_ast_node_list($5));
    $$ = node;
};
